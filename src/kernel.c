...
bool handle_page_fault(PCB* pcb);
...
int process_initialize(char *filename){
    ...
    PCB* newPCB = makePCB_with_table(file);
    error_code = load_file(fp, start, end, filename, newPCB);
    if(error_code != 0){
        fclose(fp);
        return error_code;
    }
...
int shell_process_initialize(){
    ...
    int* end = (int*)malloc(sizeof(int));
    int error_code = 0;
    error_code = load_file(stdin, start, end, "_SHELL", NULL);
    if(error_code != 0){
        return error_code;
    }
...
bool execute_process(QueueNode *node, int quanta){
    ...
    for(int i=0; i<quanta; i++){
        if (pcb->line_counter == 1) {
            if ((pcb->table_counter > 1) && (pcb->page_handled == false)) { 
               pcb->page_handled = true;
               bool b = handle_page_fault(pcb);
               if (quanta == 2) return b;
            }
            pcb->page_handled = false;
            pcb->line_counter = 2;
            line = mem_get_value_at_line(pcb->page_table[pcb->table_counter], 1);    
            pcb->PC++;
            in_background = true;
            if(pcb->priority) {
                pcb->priority = false;
            ...
            parseInput(line);
            in_background = false;
        } else if (pcb->line_counter == 2) {
            pcb->line_counter = 3;
            line = mem_get_value_at_line(pcb->page_table[pcb->table_counter], 2);    
            pcb->PC++;
            in_background = true;
            if(pcb->priority) {
                pcb->priority = false;
            ...
            pcb->line_counter = 1;
            line = mem_get_value_at_line(pcb->page_table[pcb->table_counter++], 3);    
            pcb->PC++;
            in_background = true;
            if(pcb->priority) {
                pcb->priority = false;
...
bool handle_page_fault(PCB* pcb) {
    int space = check_space_mem();
    if (space != -1) {
        load_missing_page(pcb, space);
    }else{
        evict_page();
        space = check_space_mem();
        load_missing_page(pcb, space);
    }
    return false;
}
