...
#include <sys/types.h> //new included library for rmdir
...
int quit(){
	...
	threads_terminate();
	ready_queue_destory();
	system("rm -rf backing_store_dir");
	exit(0);
}
...
int run(char* script){                                   // have not touched run, not in testcases so don't touch it for now !
	//errCode 11: bad command file does not exist
	int errCode = 0;
	char command[30] = "cp ";
    strcat(command, script);
	strcat(command, " backing_store_dir");
	system(command);
...
int exec(char *fname1, char *fname2, char *fname3, char* policy, bool background, bool mt){
	// copy all scripts in backing store directory
	if(fname1 != NULL){
		file_parsing(fname1);
		char command[30] = "cp ";
        strcat(command, fname1);
		strcat(command, " backing_store_dir");
		system(command);
    }
    if(fname2 != NULL){
